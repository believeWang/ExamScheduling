...
public class GoogleCheck extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		URL urUserInfo = new URL(
				"https://www.googleapis.com/oauth2/v3/userinfo?access_token="
						+ token);
		HttpURLConnection connObtainUserInfo = (HttpURLConnection) urUserInfo
				.openConnection();
		...
		if (connObtainUserInfo.getResponseCode() == HttpURLConnection.HTTP_OK) {
			...
			BufferedReader reader = new BufferedReader(new InputStreamReader(
					connObtainUserInfo.getInputStream(), "utf-8"));
			String strLine = "";
			while ((strLine = reader.readLine()) != null) {
		...
		List<EmployeeVO> employee = null;//ERRORRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR衝突請刪掉
		if (employee.size() == 0) {
			out.println("您非本公司員工");
		} else {
