...
public class ExamReview extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		initReview(request);
		request.getRequestDispatcher("/WEB-INF/quiz_front/examReview.jsp").forward(request, response);
	...
	private void initReview(HttpServletRequest request){
		// String examno=(String)session.getAttribute("examno");
				// 測試用
				String examno = "Java1";
		List<ExamDetailVO> questionList =(List<ExamDetailVO>) request.getServletContext().getAttribute(examno);
		request.setAttribute("questionList", questionList);
		//for test
//		Map<Long,boolean []>answerMap=( Map<Long,boolean []>) request.getSession().getAttribute("answerMap");
